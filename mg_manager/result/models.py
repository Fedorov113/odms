...
import json
from ..models import MgFile, MgSampleContainer, MgSample, DatasetHard
...
def get_mg_sample_container(df, sample, preproc, create=False):
    try:
        cont = MgSampleContainer.objects.get(
            preprocessing=preproc,
            mg_sample__name_on_fs=sample,
            mg_sample__dataset_hard__df_name=df
        )
        return cont
    except MgSampleContainer.DoesNotExist:
        if create:
            sample = get_mg_sample(df, sample)
            if sample is not None:
                cont = MgSampleContainer(mg_sample=sample, preprocessing=preproc)
                cont.save()
                return cont
    return None
...
class GeneralResult:
    ...
    def save(self):
        ...
        if self.name == 'tmtic':
            ...
            input_obj = self.input_objects['MgSampleContainer']
...
class Mp2Result(models.Model):
    mg_container = models.ForeignKey(MgSampleContainer, on_delete=models.CASCADE)
